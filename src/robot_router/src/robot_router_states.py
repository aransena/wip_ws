...
def process_result(result):
    if result == MB_SUCCESS:  ## success
        at_waypoint()
        nav_msg_pub.publish("NAV I'm at my goal location!")
        return 'goal_reached'
    elif result == MB_ABORTED:
        nav_msg_pub.publish("NAV I'm lost, please help")
        return 'new_goal'
    elif result == MB_REJECTED:
        nav_msg_pub.publish("NAV New goal received")
        return 'intervention'
    else:
        return 'error'
...
class navigate(smach.State):
    ...
    def execute(self, userdata):
        ...
        while (self.client.wait_for_result(rospy.Duration(1.0)) != True):
            ...
            nav_msg = "NAV I'm on my way... Goal: " +loc_name+" Speed: " + str(control_level-1)
            nav_msg_pub.publish(nav_msg)
            if control_level <= 1:
...
class monitor(smach.State):
    ...
    def execute(self, userdata):
        ...
        while not self.client.wait_for_result(rospy.Duration(1.0)):
            nav_msg = "NAV I'm on my way... \nGoal: " +loc_name+"\nSpeed: " + str(control_level-1)
            nav_msg_pub.publish(nav_msg)
            print responses[self.client.get_state()]
            if control_level <= 1:
                self.client.cancel_all_goals()
